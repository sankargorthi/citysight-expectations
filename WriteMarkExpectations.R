...
dbGetQuery(dbhandle,paste("DELETE FROM ", config$db, ".dbo.MARKPREDICTION WHERE DATE=\'",
    (Sys.Date() - 1),"\'", sep=""))

flog.info("Bulk inserting mark expecation data", name="quiet")
dbGetQuery(dbhandle, paste("BULK INSERT ", config$db, ".dbo.MARKPREDICTION FROM
    '/opt/citysight-expectations/markExpToday.csv'
    WITH (FIELDTERMINATOR = ',', ROWTERMINATOR = '0x0a')",
    sep=""))

dbGetQuery(dbhandle, paste("IF OBJECT_ID('MARKPREDICTIONCONVERTED', 'U') IS NOT NULL DROP TABLE ",
    config$db, ".dbo.MARKPREDICTIONCONVERTED", sep=""))

dbGetQuery(dbhandle, paste("SELECT * INTO ", config$db, ".dbo.MARKPREDICTIONCONVERTED FROM ", config$db,
